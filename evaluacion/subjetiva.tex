...
con 31 preguntas que se utilizan para validar las hipótesis\revisar{Referencia a
    donde esta}. Las preguntas están agrupadas en dos, el primer grupo cuenta
con 27 preguntas cerradas, es decir de una sola respuesta en una lista de
opciones, el segundo grupo cuenta con 4 preguntas abiertas\revisar{Qué es?}.

\observacion{Se presenta al grupo de control la prueba subjetiva?}
...
responder cualquier duda\revisar{Sobre el formato?}.
...
\observacion{Esto tiene que reflejar en los objetivos de la tesis}
...
\observacion{Los tres primeros son muy especificos}
...
\observacion{Tener en claro para objetivos del trabajo}
\begin{description}
...
\replantear{ Este factor esta relacionado con lo útil que puede ser la solución
    como herramienta de apoyo al proceso de aprendizaje de los estudiantes de
    enfermería. 
...
\item[Simulación para complementar el estudio en clase y laboratorio] se
    \replantear{refiere a que tanto las herramientas} alternativas como la
...
\item[Interacción con el paciente] se refiere a si el hecho de que el jugador
    ...
    practica\revisar{Mejor que el maniqui? lo puede complicar por que no se
        midio esto.}.
...
\item[Detalles de los pasos realizados incorrectamente] \replantear{se
        refiere a que tan importante es para el jugador que la solución no
        solo le diga los pasos que hizo de manera incorrecta sino también
        las causas por las cuales no los realizo correctamente.}

\item[Suficiencia de los detalles de los pasos realizados
    incorrectamente]\revisar{Retroalimentación suficiente respecto a los pasos
        realizados} se refiere a sí son suficientes las justificaciones breves
    acerca de las causas por las cuales que realizo incorrectamente un paso.

\item[Iconos para representar el estado del jugador]\revisar{Representacion
        iconográfica de conceptos y acciones en la UI} se refiere a la
...
\observacion{Ver formato, cambiar el description por itemize}
\begin{description}
